...
- 到顶瞬时速度误差和整段平均速度误差均不得超过 0.06 m/s；速度跟踪使用更尖锐的
  误差核，并单独惩罚整段命令误差和超过 `1.15 * command + 0.02` 的超速，防止用
  0.3 m/s 以上速度执行 0.18 m/s 命令后仍被判为成功。
...
训练从 level 0 开始。连续两次满足以下条件才升级：

- 根部 X 已越过顶平台成功线；
- 脚下地形最大高度达到该楼梯顶高（允许 1.2 cm 采样误差）；
- 躯干保持直立、至少一只脚稳定支撑，且同一步没有 base 碰撞或高度跌倒。

连续两次跌倒、停滞或未到顶 timeout 才降一级；中间出现其他结果会清零连续计数。最高一级
...
`rew_stairs_*`、`stairs_success_rate`、terrain level、value/surrogate/entropy、
...
程序同时保存 CSV 和 JSON，包含成功率、平均前进距离、平均爬升、平均存活时间、
跌倒率、到顶率、停滞率、第一阶踏上率和最大稳定落脚高度；JSON 还记录解析后的
checkpoint 绝对路径。“到顶”只要求

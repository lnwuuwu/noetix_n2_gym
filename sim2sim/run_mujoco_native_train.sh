...
case "${MODE}" in
    smoke)
        exec "${PYTHON_BIN}" -u sim2sim/train_stairs_mujoco.py \
            ...
            "${WARM_START_ARGS[@]}" \
            --device="${TRAIN_DEVICE}" \
            --run_name="mujoco_curriculum_v13_smoke${RUN_VARIANT}_s${TRAIN_SEED}" \
            --seed="${TRAIN_SEED}"
        ;;
    guide-check)
    ...
    pilot|long|recover|retune|climb-long)
        ...
        if [[ "${MODE}" == "pilot" ]]; then
            ...
            RUN_NAME="mujoco_curriculum_v13_pilot${RUN_VARIANT}_s${TRAIN_SEED}"
        elif [[ "${MODE}" == "long" ]]; then
            TARGET_ITERATIONS=3000
            RUN_NAME="mujoco_curriculum_v13_long${RUN_VARIANT}_s${TRAIN_SEED}"
        else
            if [[ "${NO_WARM_START}" == "1" ]]; then
        ...
        PID_FILE="${LOG_ROOT}/mujoco_curriculum_v13_s${TRAIN_SEED}.pid"
    ...
    resume)
        ...
        STAMP="$(date +%m%d_%H-%M-%S)"
        TRAIN_LOG="${LOG_ROOT}/mujoco_curriculum_v13_resume_s${TRAIN_SEED}_${STAMP}.log"
        PID_FILE="${LOG_ROOT}/mujoco_curriculum_v13_s${TRAIN_SEED}.pid"
        ...
        nohup "${PYTHON_BIN}" -u sim2sim/train_stairs_mujoco.py \
            ...
            --critic_symmetry_loss_coeff=0.05 \
            --device="${TRAIN_DEVICE}" \
            --run_name="mujoco_curriculum_v13_resume_s${TRAIN_SEED}" \
            --seed="${TRAIN_SEED}" \
            >"${TRAIN_LOG}" 2>&1 </dev/null &
        TRAIN_PID=$!
        printf '%s\n' "${TRAIN_PID}" >"${PID_FILE}"
    ...
    status)
        ...
        LATEST_LOG="$(find "${LOG_ROOT}" -maxdepth 1 -type f \
            -name "mujoco_curriculum_v[0-9]*_*s${TRAIN_SEED}*.log" \
            -printf '%T@ %p\n' 2>/dev/null \
            | sort -nr | head -n 1 | cut -d' ' -f2-)"
    ...
    view)
        ...
        if [[ -z "${LATEST_BEST}" ]]; then
            LATEST_BEST="$(find "${ROOT_DIR}/logs_mujoco/n2_stairs_walk" \
                ...
                | sort -nr | head -n 1 | cut -d' ' -f2-)"
            if [[ -z "${LATEST_BEST}" ]]; then
                echo "No accepted or progress-best native model exists yet." >&2
                exit 2
            fi
